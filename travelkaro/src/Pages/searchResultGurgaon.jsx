...
import HotelItemGurgaon from '../component/HotelItemGugaon'
...
const getProd = (page,limit,sort="asc",filter) => {
  return fetch(`https://bookit-com-deploye.onrender.com/gurgaon?_sort=price2&_order=${sort}&_page=${page}&_limit=${limit}&review1=${filter}`)
  .then((res)=>res.json())
}
const SearchResultGurgaon = () => {
    ...
    <div style={{backgroundColor:"#f0f3f5"}}>
      ...
      <Box w={"80%"} border="0px solid black" margin={"auto"}>
        <Box margin="auto" marginTop={20}> 
          <Heading>Searched Results For Gurgaon</Heading>
        </Box>
        ...
        <Grid border={"0px solid black"} gap={"50px"} w={"20%"} h="fit-content">
          ...
          <Grid border={"0px solid blue"} w="80%" gap={"20px"}>
            {loading ? <Loader/> : <HotelItemGurgaon hotel={searchData && searchData} />}
          </Grid>
        </Flex>
       </Box>
...
export default SearchResultGurgaon
